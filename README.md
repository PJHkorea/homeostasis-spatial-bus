...
하드웨어 기반 실시간 이동체 추적 인프라 쪽에 가깝지만 이해를 위해 편의상 게임이라는 커버로 설명하는 poc로 정의하겠습니다.

ex) 10만 명의 플레이어가 동시에 광역 스킬을 난사해도 서버 메모리 증가량을 최소화하고, 서버 렉도 허용하지 않는 메타버스/게이밍 동기화 인프라 관련 poc
